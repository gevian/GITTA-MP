...
StretchEditControls.prototype.initializeControls = function() {
  ...
  var p1 = {
    source: 1.0 * this.stretchWidget.maxSource,
    target: 1.0 * this.stretchWidget.maxSource
  };
  ...
  var h0 = {
    source: 0.25 * this.stretchWidget.maxSource,
    target: 0.25 * this.stretchWidget.maxSource
  };
  var h1 = {
    source: 0.75 * this.stretchWidget.maxSource,
    target: 0.75 * this.stretchWidget.maxSource
  };
...
StretchEditControls.prototype.drawControlPoints = function() {
    ...
    d3.select(this)
      .attr("cx", function() {
        if (i != 0 && i != _this.controlPoints.length - 1) {
          ...
          _this.controlPoints[i].source = _this.clamp(
            _this.controlPoints[i].source
          );

          var deltaXActual = _this.controlPoints[i].source - _this.tmpSource;

          if (_this.handle1Idx > 0) {
            _this.handles[_this.handle1Idx].source = _this.clamp(
              _this.tmpHandle1Source + deltaXActual
            );
          }

          if (_this.handle2Idx < _this.handles.length) {
            _this.handles[_this.handle2Idx].source = _this.clamp(
              _this.tmpHandle2Source + deltaXActual
            );
          }
      ...
      .attr("cy", function() {
        if (i != 0 && i != _this.controlPoints.length - 1) {
          ...
          _this.controlPoints[i].target = _this.clamp(
            _this.controlPoints[i].target
          );

          var deltaYActual = _this.controlPoints[i].target - _this.tmpTarget;

          if (_this.handle1Idx > 0) {
            _this.handles[_this.handle1Idx].target = _this.clamp(
              _this.tmpHandle1Target + deltaYActual
            );
          }

          if (_this.handle2Idx < _this.handles.length) {
            _this.handles[_this.handle2Idx].target = _this.clamp(
              _this.tmpHandle2Target + deltaYActual
            );
          }
  ...
  this.g.selectAll(".control-point").remove();
...
StretchEditControls.prototype.drawHandles = function() {
  ...
  this.g.selectAll(".handle-point").remove();
...
StretchEditControls.prototype.addControlPoint = function(source, target) {
  ...
  var [leftHandleX1Idx, leftHandleX2Idx] = this.circleIdx2handleIdx(
    insertSource - 1
  );
  var [rightHandleX1Idx, rightHandleX2Idx] = this.circleIdx2handleIdx(
    insertSource
  );
...
StretchEditControls.prototype.computeStretchPoints = function() {
  ...
  for (var i = 0; i < n_samples + 1; i++) {
    ...
    var line = {
      x1: Math.round(p.source),
      x2: Math.round(p.source),
      y1: 0,
      y2: p.target
    };
    var pts = [];
...
StretchEditControls.prototype.line_line_intersect = function(line1, line2) {
  ...
  if (pt_denom == 0) {
    ...
    if (
      this.btwn(pt.source, x1, x2) &&
      this.btwn(pt.target, y1, y2) &&
      this.btwn(pt.source, x3, x4) &&
      this.btwn(pt.target, y3, y4)
    ) {
      return pt;
    } else {
      return "not in range";
...
export default StretchEditControls;
