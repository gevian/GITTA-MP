...
StretchWidget.prototype.drawGrid = function(maxSource, minTarget, maxTarget) {
  ...
  var xLine = d3
    ...
    .scale(x)
    .tickFormat("")
    .tickSize(-this.widgetHeight);
  var yLine = d3
    .axisRight()
    .scale(y)
    .tickFormat("")
    .tickSize(-this.widgetWidth);
  ...
  this.svg
    ...
    .attr(
      "transform",
      "translate(" + -this.margin + "," + this.widgetHeight / 2 + ")"
    )
    .append("text")
    .attr("text-anchor", "middle")
...
StretchWidget.prototype.setStretchPoints = function(stretchPoints) {
  ...
  for (var i = 0; i < this.numStretchPoints + 1; i++) {
    stretchInstructions.push(
      this.diagram2stretched(this.stretchPoints[i]).target
    );
  }
...
StretchWidget.prototype.startStretchAnimations = function(name, duration) {
  ...
  for (var i = 0; i < this.numStretchPoints + 1; i++) {
    pts[i].changePerSecond =
      (pts[i].target - this.stretchPoints[i].target) / duration;

    if (Math.abs(pts[i].changePerSecond) < this.e) pts[i].finished = true;
    else pts[i].finished = false;
  ...
  if (name == "identity") this.setState("unstretching");
  else this.setState("stretching");
  ...
  for (var i = 0; i < this.numStretchPoints + 1; i++) {
    ...
    if (instruction.finished) {
      newPoints.push({
        ...
        target: instruction.currentTarget
      });
    } else {
      ...
      if (
        ...
        instruction.currentTarget >= instruction.target
      ) {
