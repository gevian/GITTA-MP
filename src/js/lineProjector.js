...
function LineProjector(glcanvas, surface, earth, projectionCenter) {
  ...
  glcanvas.glContainer.addEventListener(
    "mousedown",
    this.setManualProjectionLine.bind(this),
    true
  );

  this.manualLine = new ProjectionLine(
...
ProjectionLine.prototype.update = function() {
  ...
  var torusLocationV3 = new THREE.Vector3(-this.target.x, 0.0, -this.target.z)
    .normalize()
    .multiplyScalar(this.projectionCenter.scale);
  var torusLocationV4 = new THREE.Vector4(
    torusLocationV3.x,
    torusLocationV3.y,
    torusLocationV3.z,
    1.0
  ).applyMatrix4(this.projectionCenter.lightCenter.matrixWorld);
  var torusGlobalLocationV3 = new THREE.Vector3(
    torusLocationV4.x,
    torusLocationV4.y,
    torusLocationV4.z
  );

  var directionVector = this.target
    .clone()
    .sub(torusGlobalLocationV3)
    .normalize();

  var auxiliaryRaycaster = new THREE.Raycaster(
    torusGlobalLocationV3,
    directionVector.clone()
  );

  var auxiliaryIntersectsSurface = auxiliaryRaycaster.intersectObject(
    this.surface.mesh
  );
  var auxiliaryIntersectsEarth = auxiliaryRaycaster.intersectObject(
    this.earth.earthMesh
  );

  // neither intersection with globe nor with surface
  if (
    auxiliaryIntersectsEarth.length > 0 &&
    auxiliaryIntersectsSurface.length == 0
  ) {
    this.lineGeometry.vertices[0] = torusGlobalLocationV3
      .clone()
      .add(directionVector.clone().multiplyScalar(1000));
  } else {
    // calculate farest intersecton point
    var intersectionsAll = auxiliaryIntersectsEarth.concat(
      auxiliaryIntersectsSurface
    );
